...
// SI multipliers
const SI_MULTIPLIERS = { q:1e-30,r:1e-27,y:1e-24,z:1e-21,a:1e-18,f:1e-15,p:1e-12,n:1e-9,u:1e-6,µ:1e-6,m:1e-3,'':1,k:1e3,M:1e6,G:1e9,T:1e12,P:1e15,E:1e18,Z:1e21,Y:1e24,R:1e27,Q:1e30 };

// SI output for display
const SI_OUTPUT = [
  {suffix:'Q', value:1e30},{suffix:'R', value:1e27},{suffix:'Y', value:1e24},{suffix:'Z', value:1e21},
  {suffix:'E', value:1e18},{suffix:'P', value:1e15},{suffix:'T', value:1e12},{suffix:'G', value:1e9},
  {suffix:'M', value:1e6},{suffix:'k', value:1e3},{suffix:'', value:1},{suffix:'m', value:1e-3},
  {suffix:'µ', value:1e-6},{suffix:'n', value:1e-9},{suffix:'p', value:1e-12},{suffix:'f', value:1e-15},
  {suffix:'a', value:1e-18},{suffix:'z', value:1e-21},{suffix:'y', value:1e-24},{suffix:'r', value:1e-27},
  {suffix:'q', value:1e-30}
];

function updateLegendNumbers(){
  const fieldsets = document.querySelectorAll('#motor-sets fieldset');
  fieldsets.forEach((fs,index)=>fs.querySelector('legend').textContent=`Set ${index+1}`);
}
...
function validateInput(input,type){
  ...
  if(type==='power'){
    if(isNaN(parsePowerInput(value))||parsePowerInput(value)<=0){valid=false;message='Invalid power (number + SI)';}
  } else if(type==='voltage'){
    const v=parseFloat(value);if(isNaN(v)||v<=0){valid=false;message='Invalid voltage';}
  } else if(type==='efficiency'){
    const v=parseFloat(value);if(isNaN(v)||v<=0||v>100){valid=false;message='0–100% only';}
  } else if(type==='pf'){
    const v=parseFloat(value);if(isNaN(v)||v<=0||v>100){valid=false;message='0–100% only';}
  }
  let warn=input.nextElementSibling;
  if(!warn||!warn.classList.contains('warning')){
    warn=document.createElement('div');warn.classList.add('warning');warn.style.color='red';warn.style.fontSize='0.9em';
    input.insertAdjacentElement('afterend',warn);
  }
  warn.textContent=valid?'':message;
  return valid;
}
...
function createCalculationSet(){
  ...
  // Remove set
  container.querySelector('.remove-set').addEventListener('click',e=>{e.preventDefault();container.remove();updateLegendNumbers();});

  // Input validation
  container.querySelectorAll('input').forEach(input=>{
    input.addEventListener('input',()=>{validateInput(input,input.className);calculateSet(container);});
  });

  // Load type logic
  const loadType=container.querySelector('.load_type');
  const effLabel=container.querySelector('label:nth-of-type(6)');
  const effInput=container.querySelector('.efficiency');
  const pfLabel=container.querySelector('label:nth-of-type(7)');
  const pfInput=container.querySelector('.pf');

  function updateLoadFields(){
    if(loadType.value==='R'){
      effLabel.style.display='none'; effInput.style.display='none';
      pfLabel.style.display='none'; pfInput.style.display='none';
    } else{
      effLabel.style.display=''; effInput.style.display='';
      pfLabel.style.display=''; pfInput.style.display='';
    }
  }
  updateLoadFields();
  loadType.addEventListener('change',updateLoadFields);
...
function calculateSet(fieldset){
  ...
  let validAll=true;
  inputs.forEach(i=>{if(!validateInput(i,i.className)) validAll=false;});
  if(!validAll){fieldset.querySelector('.result-line').textContent='';return;}
  ...
  fieldset.querySelector('.result-line').textContent=`Current: ${formatSI(I)} A`;
}
...
function exportToPDF(){
  ...
  let y=30,setNumber=1;
  document.querySelectorAll('#motor-sets fieldset').forEach(set=>{
